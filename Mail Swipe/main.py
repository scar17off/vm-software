...
def checkMails():
    ...
    length = len(req)
    if length == 0:
        print_statusline("Your mailbox is empty. Hold tight. Mailbox is refreshed automatically every second.")
    else:
        idList = []
        for i in req:
            for k,v in i.items():
        ...
        print_statusline(f"You received {length} {x}. (Mailbox is refreshed automatically every second.)")
